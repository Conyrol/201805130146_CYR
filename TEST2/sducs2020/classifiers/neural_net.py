...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        hidden_layer1 = np.maximum(np.dot(X, W1) + b1, 0)
        hidden_layer2 = np.maximum(np.dot(hidden_layer1, W2) + b2, 0)
        ...
        #############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        res = scores - np.max(scores, axis = 1, keepdims = True)
        loss = np.log(np.exp(res).sum(axis = 1)).sum() - res[range(X.shape[0]), y].sum()
        loss = loss / X.shape[0] + 1/2 * reg * (np.sum(W1 * W1) + np.sum(W2 * W2) + np.sum(W3 * W3))
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            indexList = np.random.choice(num_train, batch_size, replace=True)
            X_batch = X[indexList]
            y_batch = y[indexList]
