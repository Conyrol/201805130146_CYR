...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        out = gamma * x_hat  + beta
        cache =(x, x_hat, gamma, beta, eps, running_mean, running_var)
...
def batchnorm_backward_alt(dout, cache):
    ...
    x, x_hat, gamma, beta, eps, mean, var = cache
    ...
    dvar = np.sum(-0.5 * dx_hat * xmu * np.power((var + eps), -1.5), axis = 0)
    dmean = np.sum(-dx_hat * istd, axis = 0) + np.sum(-2 * xmu * dvar, axis = 0) / m
    dx = dx_hat * istd + 2 * dvar * xmu / m + dmean / m
